...
function Filger:DisplayActives()
	...
	local function sortTable(a, b)
		if not C["Filger"].TestMode and C["Filger"].Expiration == true and a.data.filter == "CD" then -- Avoid an error
...
function Filger:OnEvent(event, unit, _, castID)
	...
	elseif event == "PLAYER_ENTERING_WORLD" or event == "SPELL_UPDATE_COOLDOWN" then
		...
		for i = 1, #C["FilgerSpells"][K.Class][self.Id], 1 do
			...
			if event ~= "SPELL_UPDATE_COOLDOWN" then
				local isTalent
				if not K.Classic then
					isTalent = data.talentID and select(10, GetTalentInfoByID(data.talentID))
				end
				if (data.filter == "BUFF" or data.filter == "DEBUFF") and (not data.spec or data.spec == K.Spec) and (not data.talentID or isTalent) then
