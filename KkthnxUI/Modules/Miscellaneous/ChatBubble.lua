	...
	for bubble in pairs(bubbles) do
		...
		if bubble:IsShown() and msg and (msg ~= "") then
			...
			if blizzTextWidth and blizzTextHeight and point and rpoint and blizzX and blizzY then
				...
				local ourX = math_floor((blizzX - blizzTextWidth / 2) / scale - (ourTextWidth - blizzTextWidth) / 2) -- chatbubbles are rendered at BOTTOM, WorldFrame, BOTTOMLEFT, x, y
				local ourY = math_floor(blizzY / scale - (ourTextHeight - blizzTextHeight) / 2) -- get correct bottom coordinate
				...
				bubbles[bubble]:SetBackdropBorderColor(r * 0.4, g * 0.4, b * 0.4, 0.8)
...
function Updater:InitBubble(bubble)
	...
	local space = getPadding()
	bubbles[bubble] = CreateFrame("Frame", nil, Module.BubbleBox)
	bubbles[bubble]:Hide()
...
function Module:UpdateBubbleDisplay()
	...
	if (instanceType == "none") then
		...
		Module.Updater:SetScript("OnUpdate", Module.Updater.OnUpdate)
	else
		if UnitAffectingCombat("player") then
			SetCVar("chatBubbles", 0)
		else
		...
		Module.Updater:SetScript("OnUpdate", nil)
	end
end

function Module:CreateChatBubble()
	if C["Skins"].ChatBubbles == true or not K.CheckAddOnState("NiceBubbles") then
		...
		Module.Updater.BubbleBox = Module.BubbleBox
		...
		K:RegisterEvent("PLAYER_ENTERING_WORLD", Module.UpdateBubbleDisplay)

		Module:UpdateBubbleDisplay()
		Module.BubbleBox:Show()
		for bubble in pairs(bubbles) do
			Module.Updater:HideBlizzard(bubble)
		end
	else
		Module:UpdateBubbleDisplay()
		Module.BubbleBox:Hide()
		for bubble in pairs(bubbles) do
			Module.Updater:ShowBlizzard(bubble)
		end
	end
end