...
function Module:SetupImprovedTrainerWindow()
	...
	-- Get frame textures
	local regions = {
		_G["ClassTrainerFrame"]:GetRegions()
	}
...
function Module:CreateImprovedTrainerWindow()
	if IsAddOnLoaded("Leatrix_Plus") or C["Misc"].ImprovedTrainerWindow ~= true then
		return
	end
	-- Run function when Trainer UI has loaded
	if IsAddOnLoaded("Blizzard_TrainerUI") then
		Module:SetupImprovedTrainerWindow()
	else
