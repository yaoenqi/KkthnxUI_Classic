local K, _, L = unpack(select(2, ...))

if GetLocale() == "ruRU" then
	L["%s to dismount."] = "%s чтобы спешиться."
	L["%s to leave the vehicle."] = "%s чтобы покинуть транспорт."
	L["AP"] = "AP:" --Перевести
	L["AddOn Info Text"] = "|cffffff00Следующие команды применяют настройки для поддерживаемых аддонов.|r\n\n|cff00ff00/settings dbm|r, чтобы применить настройки для |cff00ff00DeadlyBossMods.|r\n|cff00ff00/settings msbt|r, чтобы применить настройки для |cff00ff00MikScrollingBattleText.|r\n|cff00ff00/settings skada|r, чтобы применить настройки для |cff00ff00Skada.|r\n|cff00ff00/settings bt4 or bartender|r, чтобы применить настройки для |cff00ff00Bartender4.|r\n|cff00ff00/settings buggrabber|r, чтобы применить настройки для |cff00ff00!BugGrabber.|r\n|cff00ff00/settings bugsack|r, чтобы применить настройки для |cff00ff00BugSack.|r\n|cff00ff00/settings bugsack|r, чтобы применить настройки для |cff00ff00BugSack.|r\n|cff00ff00/settings pawn|r, чтобы применить настройки для |cff00ff00Pawn.|r\n|cff00ff00/settings bigwigs|r, чтобы применить настройки для |cff00ff00BigWigs.|r\n|cff00ff00/settings all|r, чтобы применить настройки для всех поддерживаемых аддонов, если они включены.\n\n"
	L["AddOn Profiles"] = "Все настройки для поддерживаемых аддонов включены!"
	L["Aerie Peak"] = "Заоблачный пик"
	L["Arathi Highlands"] = "Нагорье Арати"
	L["Ashenvale"] = "Ясеневый лес"
	L["Astranaar"] = "Астранаар"
	L["Attach List"] = "Список вложений:"
	L["Auberdine"] = "Аубердин"
	L["Azshara"] = "Азшара"
	L["BFA Invasion"] = "Faction Assults" --Перевести? Мы в Классике же..
	L["Badlands"] = "Бесплодные земли"
	L["Bandwidth"] = "Ширина канала"
	L["Bank"] = "Банк"
	L["Bars"] = "Полосы"
	L["Blackfathom Deeps"] = "Непроглядная Пучина"
	L["Blackrock Depths"] = "Глубины Черной горы"
	L["Blackrock Mountain"] = "Черная гора"
	L["Blasted Lands"] = "Выжженные земли"
	L["Bloodvenom Post"] = "Застава Отравленной Крови"
	L["Boat to"] = "Корабль в"
	L["Booty Bay"] = "Пиратская бухта"
	L["Brackenwall Village"] = "Деревня папоротников"
	L["Burning Steppes"] = "Пылающие степи"
	L["CVars Installed"] = "Установить CVars"
	L["Camp Mojache"] = "Лагерь Мохаче"
	L["Camp Taurajo"] = "Лагерь Таурахо"
	L["Can't Buy Slot"] = "Больше нельзя купить слотов!"
	L["Cenarion Hold"] = "Крепость Кенария"
	L["Central Mesa"] = "Громовой Утес"
	L["Changelog"] = "Изменения"
	L["Changes Reload"] = "Для применения настроек необходимо перезагрузить интерфейс."
	L["Character Settings"] = "Настройки персонажа"
	L["Chat Installed"] = "Настроить чат"
	L["Chillwind Camp"] = "Лагерь промозглого ветра"
	L["Collect All"] = "Открыть всё"
	L["Collect Gold"] = "Собрать золото"
	L["Collect Letters"] = "Получить"
	L["Combat"] = "Бой"
	L["Copy Chat"] = "Скопировать чат"
	L["Count"] = "Количество"
	L["Current Invasion"] = "Текущий: "
	L["Darkshire"] = "Темнолесье"
	L["Darkshore"] = "Темные берега"
	L["Delete Grays"] = "Удалить серые"
	L["Delete Mode Enabled"] = "|nМожно уничтожить предмет, зажав CTRL+ALT. Качество предмета должно быть ниже Редкого (синее)."
	L["Desolace"] = "Пустоши"
	L["Disable to hide areas."] = "В выключенном состоянии скроет|nнеисследованные области на карте."
	L["Disband Group"] = "Распустить группу"
	L["Discord URL"] = "https://discord.gg/YUmxqQm"
	L["Discord"] = "|cff7289daDiscord|r"
	L["Download"] = "Загрузить"
	L["DuelCanceled_Pet"] = "Запрос на битву питомцев от %s отклонён."
	L["DuelCanceled_Regular"] = "Запрос на дуэль от %s отклонён."
	L["Dungeon"] = "Подземелье"
	L["Durotar"] = "Дуротар"
	L["Duskwood"] = "Сумеречный лес"
	L["Dustwallow Marsh"] = "Пылевые топи"
	L["Dwarven District"] = "Квартал Дворфов"
	L["Eastern Plaguelands"] = "Восточные Чумные земли"
	L["Emotes"] = "Эмоции в чате"
	L["Empty Slot"] = "Пустой слот"
	L["Enable to show hidden areas."] = "Во включенном состоянии раскроет|nнеисследованные области на карте."
	L["Equipement Set"] = "Набор экипировки"
	L["Everlook"] = "Круговзор"
	L["Experience"] = "Опыт"
	L["Favourite Mode Enabled"] = "|nТеперь можно добавить предметы в избранное.|nЕсли настройка 'Включить фильтр Предпочтений' включена, предмет по которому вы кликните, добавиться в раздел Предпочтения.|nРежим недоступен для серого мусора."
	L["Favourite Mode"] = "Режим Избранного"
	L["Feathermoon Stronghold"] = "Крепость Оперенной Луны"
	L["Felwood"] = "Оскверненный лес"
	L["Feralas"] = "Фералас"
	L["Fishy Loot"] = "Fishy Loot" --Перевести
	L["FixParty"] = "\n|cff4488ff".."Если вы застряли в группе, попробуйте следующее".."|r\n\n|cff00ff001.|r Пригласите кого-нибудь в свою группу.\n|cff00ff002.|r Переделайте группу в рейд.\n|cff00ff003.|r Выйдите из группы.\n|cff00ff004.|r Пригласите друзей в свою новую группу.\n\n"
	L["Flame Crest"] = "Пламенеющий Стяг"
	L["Freewind Post"] = "Застава Вольного ветра"
	L["From"] = "От"
	L["Gadgetzan"] = "Прибамбасск"
	L["General"] = "Общее"
	L["Get Naked"] = "Двойной клик чтобы снять все вещи и остаться голым <3"
	L["Ghost"] = "Призрак"
	L["Global Settings"] = "Общие настройки"
	L["Gnomeregan"] = "Гномреган"
	L["Grom'gol Base Camp"] = "Лагерь Гром'Гол"
	L["Guild Repair"] = "Ваши вещи были отремонтированы за счёт гильдии на сумму: "
	L["Hammerfall"] = "Павший молот"
	L["Hide Undiscovered Areas"] = "Скрыть неисследованные зоны"
	L["Hillsbrad Foothills"] = "Предгорья Хилсбрада"
	L["Hold Shift/Click"] = "Зажмите Shift + Левый клик чтобы двигать"
	L["Home Latency:"] = "Задержка локальная:"
	L["Home Protocol:"] = "Протокол локальный:"
	L["Honor Remaining"] = "Чести осталось:"
	L["Honor XP"] = "Честь:"
	L["INVENTORY_CLASS_RELATED"] = "Классовые вещи"
	L["Install"] = "Установка"
	L["Ironforge"] = "Стальгорн"
	L["Item Delete Mode"] = "Режим удаления вещей"
	L["Kargath"] = "Каргат"
	L["Keybind Mode"] = "Наведите мышку на нужную кнопку на панели. Нажмите Esc или правой кнопкой мыши чтобы удалить текущее назначение."
	L["KkthnxUI Help"] = "Помощь по KkthnxUI"
	L["KkthnxUI Outdated"] = "Ваша версия KkthnxUI v"..K.Version.." устарела."
	L["Lack"] = "Не хватает"
	L["Lake Elune'ara"] = "Озеро Элуне'Ара"
	L["Lake Everstill"] = "Озеро Безмолвия"
	L["Leeeeeroy!"] = "Лииииирой!"
	L["Left Click"] = "Левый клик:"
	L["Legion Invasion"] = "Вторжение Легиона"
	L["Light's Hope Chapel"] = "Часовня Последней Надежды"
	L["Loch Modan"] = "Лок Модан"
	L["Loot"] = "Добыча"
	L["Lower Blackrock Spire"] = "Нижняя часть пика Черной горы"
	L["Lower Wilds"] = "Низинные Чащобы"
	L["Maraudon"] = "Мародон"
	L["Marshal's Refuge"] = "Укрытие Маршалла"
	L["Menethil Harbor"] = "Гавань Менетилов"
	L["Middle Click"] = "Клик средней кнопкой:"
	L["Molten Core"] = "Огненные Недра"
	L["Moonglade"] = "Лунная поляна"
	L["Morgan's Vigil"] = "Дозор Морганы"
	L["Move Blocked"] = "Вы уже видели этот ролик ранее, пропускаем."
	L["MoveUI"] = "Двигать интерфейс"
	L["Nethergarde Keep"] = "Крепость стражей пустоты"
	L["Next Invasion"] = "След.: "
	L["Nijel's Point"] = "Высота Найджела"
	L["No Guild"] = "Без гильдии"
	L["No Vendor"] = "Нужно находиться рядом с торговцем."
	L["Not Enough Money"] = "Недостаточно денег для починки."
	L["Onyxia's Lair"] = "Логово Ониксии"
	L["Open Chat Channels"] = "Открыть Каналы чата"
	L["Orgrimmar"] = "Оргриммар"
	L["Press To Copy"] = "|nНажмите <CTRL/C> чтобы скопировать."
	L["Profile"] = "Профиль "
	L["ProfileDel"] = " Удалён: "
	L["ProfileInfo"] = "\n/profile list\n/profile #\n/profile delete #\n\n"
	L["ProfileNotFound"] = "Профиль не найден"
	L["ProfileSelection"] = "Выберите профиль для загрузки (например: /profile 5)"
	L["Profiles"] = "Профили"
	L["Pull ABORTED!"] = "Отсчёт ПРЕРВАН!"
	L["Pulling In"] = "Отсчёт %s из %s.."
	L["Purchase Slot"] = "Купить слот для сумки"
	L["Ragefire Chasm"] = "Огненная пропасть"
	L["Raid Menu"] = "Меню рейда"
	L["Raid"] = "Рейд"
	L["Ratchet"] = "Кабестан"
	L["Razorfen Downs"] = "Курганы Иглошкурых"
	L["Razorfen Kraul"] = "Лабиринты Иглошкурых"
	L["Reagents"] = "Переключиться на реагенты"
	L["Redridge Mountains"] = "Красногорье"
	L["Refuge Pointe"] = "Опорный пункт"
	L["Remaining"] = "Осталось:"
	L["Repair Cost"] = "Ваши предметы были отремонтированы на сумму: "
	L["Reset CVars"] = "Сброс CVars"
	L["Reset Chat"] = "Сброс чата"
	L["Reset Details"] = "Сброс Details"
	L["Reset KkthnxUI"] = "Вы уверены что хотите сбросить все настройки этого профиля?"
	L["Restart Graphics"] = "Настройки которые вы изменили требуют перезапуска игры."
	L["Rested"] = "Отдых:"
	L["Revantusk Village"] = "Деревня Сломанного Клыка"
	L["Reveal Hidden Areas"] = "Показывать скрытые зоны"
	L["Reveal"] = "Показывать"
	L["Right Click To Search"] = "Правый клик для поиска"
	L["Right Click"] = "Правый клик"
	L["Roll"] = "Выполнить /roll"
	L["Rut'theran Village"] = "Деревня Рут'теран"
	L["Sapped"] = "Ошеломление"
	L["SappedBy"] = "Ошеломление от: "
	L["Scarlet Monastery"] = "Монастырь Алого Ордена"
	L["Scholomance"] = "Некроситет"
	L["Searing Gorge"] = "Тлеющее ущелье"
	L["Section"] = "Секция"
	L["Sentinel Hill"] = "Сторожевой холм"
	L["Shadowfang Keep"] = "Крепость Темного Клыка"
	L["Shadowprey Village"] = "Деревня Ночных Охотников"
	L["Share Your Experience"] = "Поделиться прогрессом опыта"
	L["Shift Left Click"] = "Shift + Левый клик"
	L["Show Hide Details"] = "Скрыть/показать Details"
	L["Show Hide Skada"] = "Скрыть/показать Skada"
	L["Silithus"] = "Силитус"
	L["Silverpine Forest"] = "Серебряный бор"
	L["Skip Install"] = "Пропустить установку"
	L["Sold Grays"] = "Продано серых вещей на сумму: %s"
	L["Southshore"] = "Южнобережье"
	L["Splintertree Post"] = "Застава Расщепленного Дерева"
	L["Stack Cap"] = "Stack Caps" --Перевести
	L["Step 0"] = "Thank you for choosing |cff4488ffKkthnxUI|r!|n|nYou will be guided through the installation process in a few simple steps. At each step, you can decide whether or not you want to apply or skip the presented settings."
	L["Step 1"] = "The first step applies the essential settings. This is |cffff0000recommended|r for any use unless you want to apply only a specific part of the settings.|n|nClick 'Apply' to apply the settings and 'Next' to continue the install process. If you wish to skip this step just press 'Next'."
	L["Step 2"] = "The second step applies the correct chat setup. If you are a new user this step is recommended. If you are an existing user you may want to skip this step.|n|nClick 'Apply' to apply the settings and 'Next' to continue the install process. If you wish to skip this step just press 'Next'."
	L["Step 3"] = "The third step applies the correct UI Scale setup. If you decide you do not like this you can adjust the scale in |cffffd200ESC > System > Advanced|r and you will see UI Scale slider you can adjust!"
	L["Step 4"] = "Installation is complete. Please click the 'Complete' button to reload the UI. |n|nThank you for using |cff4488ffKkthnxUI|r! You Rock!"
	L["Stonard"] = "Каменор"
	L["Stonetalon Mountains"] = "Когтистые горы"
	L["Stonetalon Peak"] = "Пик каменного когтя"
	L["Stormwind"] = "Штормград"
	L["Stranger"] = "Stranger" --Перевести
	L["Stranglethorn Vale"] = "Тернистая долина"
	L["Stratholme (Main Gate)"] = "Стратхольм (Main Gate)"
	L["Stratholme (Service Gate)"] = "Стратхольм (Service Gate)"
	L["Subzone Eastern Transept"] = "Eastern Transept" --Перевести
	L["Subzone Grand Bazaar"] = "Subzone Grand Bazaar" --Перевести
	L["Subzone Port of Zandalar"] = "Subzone Port of Zandalar" --Перевести
	L["Sun Rock Retreat"] = "Приют у Солнечного Камня"
	L["Swamp of Sorrows"] = "Болото Печали"
	L["Switch to Bank"] = "Переключиться на Банк"
	L["Talonbranch Glade"] = "Talonbranch Glade" --Перевести
	L["Talrendis Point"] = "Застава Талрендис"
	L["Tanaris"] = "Танарис"
	L["Targeted By"] = "Цель для:"
	L["Tarren Mill"] = "Мельница Таррен"
	L["Teldrassil"] = "Тельдрассил"
	L["Temple of Atal'Hakkar"] = "Храм Атал'Хаккара"
	L["The Barrens"] = "Степи"
	L["The Crossroads"] = "Перекресток"
	L["The Deadmines"] = "Мертвые Копи"
	L["The Great Forge"] = "Великая Кузня"
	L["The Hinterlands"] = "Внутренние земли"
	L["The Sepulcher"] = "Гробница"
	L["The Stockade"] = "Тюрьма"
	L["Thelsamar"] = "Телсамар"
	L["Theramore Isle"] = "Остров Терамор"
	L["Thorium Point"] = "Лагерь Братства Тория"
	L["Thousand Needles"] = "Тысяча Игл"
	L["Thunder Bluff"] = "Громовой Утес"
	L["Tinker Town"] = "Город Механиков"
	L["Tirisfal Glades"] = "Тирисфальские леса"
	L["Toggle Bags"] = "Показать Сумки"
	L["Toggle Bags"] = "Показать Сумки"
	L["Toggle KkthnxUI Config"] = "Показать окно настроек KkthnxUI"
	L["Toggle PvP"] = "Показать окно PvP"
	L["Toggle Quick Menu"] = "Показать быстрое меню"
	L["Toggle Reputation"] = "Показать окно репутации"
	L["Trade District"] = "Торговый Квартал"
	L["Trade Quarter"] = "Торговый Квартал"
	L["Trade"] = "Торговля"
	L["Trait"] = "Trait" --Перевести
	L["Tram to"] = "Метро в"
	L["Uldaman"] = "Ульдаман"
	L["Un'Goro Crater"] = "Кратер Ун'Горо"
	L["Undercity"] = "Подгород"
	L["Upper Blackrock Spire"] = "Верхняя часть пика Черной горы"
	L["Valley of Strength"] = "Аллея Силы"
	L["Valormok"] = "Храбростан"
	L["Vendor Grays"] = "Продавать серое"
	L["Wailing Caverns"] = "Пещеры Стенаний"
	L["Warning Blizzard AddOns"] = "Какой-то из ваших других аддонов отключил Blizzard_CompactRaidFrames. Это может привести к ошибкам и другим проблемам. Аддон будет переподключен."
	L["Western Plaguelands"] = "Западные Чумные земли"
	L["Westfall"] = "Западный Край"
	L["Wetlands"] = "Болотина"
	L["Whisper"] = "Шепот"
	L["Winterspring"] = "Зимние Ключи"
	L["World Latency:"] = "Задержка сервера:"
	L["World Protocol:"] = "Протокол сервера:"
	L["XP"] = "Опыт:"
	L["Zeppelin to"] = "Дирижабль в"
	L["Zoram'gar Outpost"] = "Застава Зорам'гар"
	L["Zul'Farrak"] = "Зул'Фаррак"
end
